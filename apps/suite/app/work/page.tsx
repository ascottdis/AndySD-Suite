...
export default async function WorkPage({ searchParams }: WorkPageProps) {
  ...
  const pillarParam = ((sp?.pillar as string) ?? "").toLowerCase();
    ...
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-14">
        <div className="flex items-start justify-between gap-6 flex-wrap">
          ...
          <Link href="/" className="px-4 py-2 rounded-xl border border-slate-800 bg-slate-950/40 hover:bg-slate-900/40 hover:border-slate-700 transition-colors text-slate-200">â† Home</Link>
        </div>
